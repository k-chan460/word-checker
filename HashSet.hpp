...
template <typename ElementType>
void HashSet<ElementType>::destroyAll(Node* curr){
        //Node* curr = hash[i];
        if(curr != nullptr){
            Node* prev = curr;
            delete prev;
            destroyAll(curr->next);
        }
}
...
template <typename ElementType>
void HashSet<ElementType>::addAll(Node*& curr, const ElementType& element){
    if(curr == nullptr){
        curr = new Node{element};
        hashsize++;
    }
    else{
        addAll(curr->next, element);
    }
}
...
{
    unsigned int index = hashFunction(element) % DEFAULT_CAPACITY;
    addAll(hash[index],element);
}
...
{
    if(hashFunction(element) % DEFAULT_CAPACITY == index && this->contains(element)){
        return true;
    }
    return false;
}
